class GradientColor extends RegisterableObject {
  ...
  protected colors:SolidColor[];
  protected ratios:number[];
  ...
  protected onUpdateStyle:Function;
  ...
  constructor(colors:SolidColor[]=null,ratios:number[]=null,isLinear:boolean=true){
    super();
  ...
  public clone(cloneColors:boolean=false):GradientColor{
    let c:SolidColor[];
    if(cloneColors){
      c = [];
      let i:number,len:number = this.colors.length;
  ...
  public setColorStep(colors:SolidColor[],ratios:number[]=null){
    ...
    nbStep = this.nbStep;
    for(i=0;i<nbStep;i++){
      if(this.colors && this.colors[i]) this.colors[i].removeEventListener(SolidColor.UPDATE_STYLE,this.onUpdateStyle)
  ...
  public addColorStep(ratio:number,r:number|string=0,g:number=0,b:number=0,a:number=1):SolidColor{
  ...
  public getColorById(id:number):SolidColor{return this.colors[id]}
  ...
  public getRatioById(id:number):number{return this.ratios[id]}
  public setRatioById(id:number,ratio:number){
    this.ratios[id] = ratio;

  }

  public getGradientStyle(context2D:CanvasRenderingContext2D,target:Display2D):CanvasGradient{
