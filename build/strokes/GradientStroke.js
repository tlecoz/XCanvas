class GradientStroke extends Stroke {
    ...
    apply(context, path, target) {
        if (this.dirty || this._gradient.needsUpdate) {
            ...
            GradientStroke.gradients[target.display2dName] = this._gradient.getGradientStyle(context, target);
            this.dirty = false;
        }
        context.save();
        context.fillStyle = GradientStroke.gradients[target.display2dName];
        context.stroke(path);
        context.restore();
    }
}
