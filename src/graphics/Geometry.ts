class Geometry {
  ...
  public getPoints(pathDatas:number[]):void{
    ...
    let i:number=0,len:number = pathDatas.length,NB:number,count:number,type:number,start:number;
    let o:any;
    while(i<len){
      type = pathDatas[i++];
  ...
  public arcTo(x1:number,y1:number,x2:number,y2:number,radius:number){
    ...
    let dx:number = x1 - x0;
    let dy:number = y1 - y0;
    let d:number = Math.sqrt(dx*dx+dy*dy) - radius;
    let a:number = Math.atan2(dy,dx);

    let _x0:number = x0 + Math.cos(a) * d;
    let _y0:number = y0 + Math.sin(a) * d;
