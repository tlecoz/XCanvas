...
import { BorderVectorizer } from "./vectorizer/BorderVectorizer";
import { FitCurve } from "./vectorizer/FitCurve";
...
export class BorderFinder {
  ...
  public getOutsideBorder(source: BitmapData): BorderPt[] {
    ...
    var _temp: BitmapData = new BitmapData(r.w, r.h, "rgba(0,0,0,0)");
    //matrix.identity();
  ...
  public createGraphicsGeometryFromBitmapData(source: BitmapData, firstPassPrecision: number = 0.2, fitCurvePrecision: number = 1): Geometry {
  ...
  public getBorderFromBitmapData(source: BitmapData,
    ...
    this.n = 0;
    //this.old = new BorderPt(0, 0, 0);
    this.count = 0;
    //let directionDatas: number[] = BorderFinder.directionDatas;

    this.offsetX = BorderFinder.offsetX;
    this.offsetY = BorderFinder.offsetY;
    let colorTracking: boolean = trackColor;
    ...
    //let firstId: number;
    let id: number = this.firstId = (py * bd.width + px) >> 0; //(val) >> 0 = int(val)
