...
export class HolePathRemover {
  private static _instance: HolePathRemover;

  public debug: BitmapData;
  public outside: BorderPt[] | null = null;
  private grid: BorderPt[][][] | null = null;
  private cells: BorderPt[] | null = null;
  private gridSize: number = 0;
  public quads: any[] | null = null;
  ...
  public drawPath(pct: number, color: string = "#000000"): void {
    ...
    var path: BorderPt[] = this.outside as BorderPt[];
  ...
  private findClosestPoints(outside: BorderPt[], hole: BorderPt[]): { outsidePt: BorderPt | null, holePt: BorderPt | null } {

    var result: { outsidePt: BorderPt | null, holePt: BorderPt | null, dist: number } = { outsidePt: null, holePt: null, dist: 9999999999999 };
  ...
  private findClosestPoints2(hole: BorderPt[]): { outsidePt: BorderPt | null, holePt: BorderPt | null } {
    ...
    var grid: BorderPt[][][] = this.grid as BorderPt[][][];
    var size: number = this.gridSize;
    var points: any = [];
    var cells: any = this.cells;
    var o;

    for (i = 0; i < cells.length; i++) (cells[i] as any).used = false;
  ...
  public addHole(hole: BorderPt[], precision: number = 0): BorderPt[] {
    ...
    var outside: BorderPt[] = this.outside as BorderPt[];
    var id = (o.outsidePt as BorderPt).id - 1;
    if (id < 0) id += outside.length;
    var outConnectStartPt: BorderPt = outside[id];
    var outConnectEndPt: BorderPt = outside[((o.outsidePt as BorderPt).id + 1) % outside.length];
    //var outErasedPt:BorderPt = outside.splice(o.outsidePt.id,1)[0];




    var holeConnectStartPt: BorderPt = hole[(o.holePt as BorderPt).id];
    //console.log("outConnectStartPt = ",outConnectStartPt)
    //console.log("holeConnectStartPt = ",holeConnectStartPt)

    var id = (o.holePt as BorderPt).id - 1
    ...
    var first = outside[0];
    var k = 0;
    var pt: BorderPt | null = path[k++] = first;
    pt.id = 0;
    pt = pt.next as BorderPt;
    while (pt && pt != first) {
      pt.id = k;
      path[k++] = pt;
      pt = pt.next as BorderPt;
