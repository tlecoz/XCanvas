class BitmapData extends EventDispatcher {
  ...
  private static abstractContext:CanvasRenderingContext2D = BitmapData.abstractCanvas.getContext("2d");
  ...
  */
  ...
  public drawImage(img:HTMLImageElement|HTMLVideoElement|HTMLCanvasElement,srcX:number=0,srcY:number=0,srcW:number=-1,srcH:number=-1,destX:number=0,destY:number=0,destW:number=-1,destH:number=-1){
    if(srcW==-1){
      srcW = img.width;
      srcH = img.height;
    }
  ...
  public floodFillRGBAandReturnOutputCanvas(x:number,y:number,fillR:number,fillG:number,fillB:number,fillA:number=255):BitmapData{
    ...
    let bmpW:number = this.width,bmpH:number = this.height;
